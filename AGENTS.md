...
> Отчёт: `.context/phase-7-mock-data-mode.md`. Только frontend-submodule.

Тумблер **«мок-данные»** в хедере (рядом с почтой), **включён по умолчанию**: фронт сам
фабрикует данные и **не обращается** к бэкенду/движку/биржам — для демонстрации UI без
поднятого стека.

1. **Перехват на уровне api:** каждая функция в `frontend/src/api/*.ts` в начале проверяет
   `isMockEnabled()` и возвращает данные из общего mock-стора вместо `fetch`
   (`bots/trades/balances/positions/candles/credentials/exchanges/health/backtest`). Виджеты
   не тронуты — подхватывают мок на своём polling.
2. **Mock-слой** `frontend/src/mock/`: `config.ts` (флаг localStorage `crypto.mock.enabled`,
   биржи, allowlist пар + базовые цены, дефолты стратегий), `generators.ts` (рандом, свечи),
   `store.ts` — singleton `mockStore`: сессия из **5–15** стратегий, по позиции на каждую
   активную, баланс (`equity = кэш + Σ(нотионал + PnL)`), история сделок, фейковый бэктест.
3. **`MockDataContext.tsx`** (внутри `LogsProvider` в `Layout`): таймер `tick()` каждые 4с
   двигает PnL/equity, иногда рождает сделку и **строки логов** (через новый
   `LogsContext.pushLog`).
4. **Связи:** в Настройках «подключены» все биржи; статус движка показывает число фиктивных
   стратегий; стратегии **редактируемы** (новый диалог в `Strategies.tsx` → `updateBotParams`)
   и создаются на фейковом рынке; бэктест отдаёт сгенерированный результат.
5. **Тесты:** под Vitest режим по умолчанию ВЫКЛЮЧЕН (`config.ts` проверяет `process.env.VITEST`),
   чтобы прежние тесты шли реальным путём через мокнутый `fetch`. Добавлен
   `src/test/mockStore.test.ts`.
6. **Round 2:** сессия **кэшируется** в `localStorage` (`crypto.mock.session.v1`, TTL 48ч) и
...
- **Документировать проделанное (ОБЯЗАТЕЛЬНО):** после каждой значимой работы агент
  **постоянно поддерживает историю в актуальном состоянии**:
  1. дописывает блок `Phase N` в `§9 Что было проделано` этого файла (кратко, без секретов);
  2. кладёт **подробный отчёт** в `.context/phase-N-<slug>.md` (цель, архитектура, список
     тронутых файлов, как тестировать) — это «большой файл с описанием сделанного».
  Не оставляй фазу недокументированной: `§9` + отчёт в `.context/` — часть Definition of Done.
```
