...
## Round 2 — кэш сессии + реалистичный бэктест + анимация

1. **Кэш сессии (~2 суток).** Сгенерированная сессия (ключи, боты, позиции, цены, сделки,
   бэктесты) сохраняется в `localStorage["crypto.mock.session.v1"]` с `savedAt` и **переживает
   перезагрузку**; TTL 48 ч (`SESSION_TTL_MS`). `ensureSession()` сначала пытается
   `tryRestore()`, иначе генерит заново. `persist()` вызывается в `tick()` и во всех мутациях.
   Тумблер по-прежнему перезагружает страницу — но при включении подхватывается **тот же**
   кэш, а не новая сессия.
2. **Реалистичный бэктест.** `makeBacktestResult` теперь не «рисует потолок», а **симулирует**:
   random-walk цены (волатильность растёт с таймфреймом) + простая логика входов/выходов;
   метрики (доходность, max DD, Sharpe, win rate, profit factor) считаются из фактических
   сделок и equity-кривой. `win_rate` отдаётся в процентах (под формат страницы).
3. **Скорость прогона зависит от ТФ и периода.** `runBacktest` стартует job со статусом
   `running`; `getBacktest` помечает `completed` по истечении расчётного времени
   `duration = clamp(1000 + candleCount·3, 1800..12000)` мс. Мельче ТФ + длиннее период →
   больше свечей → дольше прогон.
4. **Claude-style анимация ожидания.** Новый
   `frontend/src/app/components/BacktestRunningIndicator.tsx`: текст стадии сменяется и
   мерцает (shimmer), иконка меняется вместе с ним и анимируется по-разному (spin/bob/pulse)
   на каждой стадии + вращающееся кольцо-градиент. Подключён в running-ветке `Backtesting.tsx`.
5. Тесты: +1 (всего **98**) — проверка таймингов прогона через `vi.useFakeTimers`.
